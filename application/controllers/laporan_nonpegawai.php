...
class Laporan_nonpegawai extends MY_Controller {
    public function __construct() {
        parent::__construct();
        parent::default_meta();
        $this->data->metadata = $this->template->get_metadata();
		$this->data->judul=$this->template->get_judul();
		$this->load->model(array('laporan_nonpegawai_m'));
    }    
	...
	public function getListLaporanNonpegawai(){
		$data['rows']=$this->laporan_nonpegawai_m->getListLaporanNonpegawai('rows');
		$data['total']=$this->laporan_nonpegawai_m->getListLaporanNonpegawai('total');
	...
	public function cetakLaporan($tgl_awal="",$tgl_akhir="")
	{
		...
		<htmlpagefooter name="MyFooter1">
			<table width="100%" style="vertical-align: bottom; font-family: serif; font-size: 8pt; color: #000000; font-weight: bold; font-style: italic;">
				<tr>
					<td width="33%" align="center" style="font-weight: bold; font-style: italic;">PDAM Malang - Laporan Pemeriksaan Non Pegawai, PAGE {PAGENO} dari {nbpg}</td>
				</tr>
			</table>
		</htmlpagefooter>
		...
		if($TGL_MULAI=='' ){
		$html.="
		<div style='font-size:20px; font-weight:bold; text-align:center'>Laporan Pemeriksaan Non Pegawai<br/> Periode(".date('d-m-Y')." sampai ".date('d-m-Y').")</div>"; 			
		}else{
		$html.="
		<div style='font-size:20px; font-weight:bold; text-align:center'>Laporan Pemeriksaan Non Pegawai<br/> Periode(".$tgl_awal." sampai ".$tgl_akhir.")</div>";
 		}
		$html .='
		...
		$no=1;
		$data = $this->laporan_nonpegawai_m->getLaporan($TGL_MULAI,$TGL_SELESAI);
		foreach($data as $row){
		$html .='  
		  <tr>
			<td>'.$row->nip.'</td>
